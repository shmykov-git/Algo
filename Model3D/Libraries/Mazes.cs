...
namespace Model3D
{
    public static class Mazes
    {
        ...
        public static (Shape maze, Shape holes, Shape path) CreateNet3MazeBox(int m, int n, int l, bool closed, (int i, int j, int k)[] outHoles = null, int seed = 0)
        {
            ...
            var holes = hs.Length > 0 && hs[0].j == -1 // for top only
                ? g.VisitEdges(seed, GraphVisitStrateges.SimpleRandom(seed), graph.nodes.First(n => n.i == num(hs[0].i, 0, hs[0].k))).Select(e => e.e.OrderedEdge()).ToHashSet()
            ...
            if (hs.Length == 2)
            {
                var nA = graph.Nodes.First(nI => nI == num(hs[0].i, 0, hs[0].k));
                var nB = graph.Nodes.First(nI => nI == num(hs[1].i, n - 1, hs[1].k));
            ...
            if (closed)
            {
                var wallL1 = (m, n).SelectRange((i, j) => hs.Contains((i, j, -1)) ? Shape.Empty : GetWall(new Vector3(i, j, 0), new Vector3(i, j, -1), brick, wallScale)).Aggregate((a, b) => a + b);
                var wallL2 = (m, n).SelectRange((i, j) => hs.Contains((i, j, l)) ? Shape.Empty : GetWall(new Vector3(i, j, l - 1), new Vector3(i, j, l), brick, wallScale)).Aggregate((a, b) => a + b);
                var wallN1 = (m, l).SelectRange((i, k) => hs.Contains((i, -1, k)) ? Shape.Empty : GetWall(new Vector3(i, 0, k), new Vector3(i, -1, k), brick, wallScale)).Aggregate((a, b) => a + b);
                var wallN2 = (m, l).SelectRange((i, k) => hs.Contains((i, n, k)) ? Shape.Empty : GetWall(new Vector3(i, n - 1, k), new Vector3(i, n, k), brick, wallScale)).Aggregate((a, b) => a + b);
                var wallM1 = (n, l).SelectRange((j, k) => hs.Contains((-1, j, k)) ? Shape.Empty : GetWall(new Vector3(0, j, k), new Vector3(-1, j, k), brick, wallScale)).Aggregate((a, b) => a + b);
                var wallM2 = (n, l).SelectRange((j, k) => hs.Contains((m, j, k)) ? Shape.Empty : GetWall(new Vector3(m - 1, j, k), new Vector3(m, j, k), brick, wallScale)).Aggregate((a, b) => a + b);
