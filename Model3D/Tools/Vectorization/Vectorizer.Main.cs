...
public partial class Vectorizer
{
    ...
    private Mp GetMpOuterDir((int i, int j) dir) =>
        dir switch
        {
            (0, 0) => Mp.None,
            (-1, 0) => Mp.OuterLeft,
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private Mp GetMpInnerDir((int i, int j) dir) =>
        dir switch
        {
            (0, 0) => Mp.None,
            (-1, 0) => Mp.InnerLeft,
    ...
    private Shape GetShapeFromMap(bool[][] map)
    {
        ...
        // todo: тут есть ноды для этого shape0, в них доп. информация о том какой полигон в каком находится
