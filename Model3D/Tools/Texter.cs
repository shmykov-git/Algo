...
namespace Model3D.Tools
{
    ...
    {
        public static Shape GetText(string text, int fontSize = 20, string fontName = "Arial")
        {
            var lines = text.Split("\r\n").ToArray();

            var m = 2 * fontSize * lines.Length + 1;
            var n = 2 * fontSize * lines.Max(l => l.Length) + 1;
            ...
            bool IsPoint((int i, int j) v)
            {
                ...
                return c.R < level && c.G < level && c.B < level;
            }
