    ...
    static class Scene3
    {
        public static Shape GetShape(Settings settings)
        {
            ...
            var shapes = new Shape[]
            {
                ...
                //Surfaces.Plane(30, 30).Centered().Normed().Mult(1.5).Where(v=>v.Length<1).Rotate(Rotates.Z_X).Rotate(Quaternion.FromRotation(Vector3.XAxis, new Vector3(1,1.8,0).Normalize())).Move(0.2*new Vector3(1,1.8,0)).ToLines3(0.5, Color.White),
                //Shapes.GolfBall.Mult(2).Move(4, -2, 0).ToLines3(2, Color.Red),
                Shapes.IcosahedronSp1.Mult(3).Move(3, 1, 0).ToMetaShape3(3, 3, Color.Yellow, Color.Goldenrod),
                ...
                Vectorizer.GetContentShape(settings.GetContentFileName("f2.jpg"), 180).Mult(1.5).ApplyZ(Funcs3Z.Waves).ToLines3(1, Color.Blue).ApplyColorGradient(new Vector3(8,1,0).Normalize(), null, null, null, null,null,  Color.Goldenrod, Color.Goldenrod,  Color.Goldenrod, Color.Goldenrod), // .ApplyZ(Funcs3Z.SphereMR(1.2)).Move(-0.7,-0.7,0.5)
