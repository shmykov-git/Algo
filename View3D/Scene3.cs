...
namespace View3D
{
    static class Scene3
    {
        public static Shape GetShape()
        {
            ...
            var shape = Parquets.Triangles(18, 62, 0.1).Scale((Math.PI / 3.1, 3.0.Sqrt() * 2 / 1.7)).Move((Math.PI, 0)).ToShape3().ToLines(20).AddVolumeZ(0.02).Transform(TransformFuncs3.Flower(1.5, 2, 3)).Rotate(Rotates.Z_Y).Scale(1, 0.2, 1) + Parquets.PentagonalKershner8ForTube(3, 15, 0.3).ToShape3().ToLines(40).AddVolumeZ(0.05).Transform(TransformFuncs3.CylinderWrapZ).Rotate(Rotates.Z_Y).Mult(0.1).Move(0, -2.9, 0);
